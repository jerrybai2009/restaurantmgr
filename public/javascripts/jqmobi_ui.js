function SwipeHandle(q, f) {
    ...
    function r() {
        ...
        if (f.isOpenNextAndPrevious) {
            ...
            J.style.top = (q.clientHeight - 35) / 2 + "px";
            var H = document.createElement("div");
            H.className = "nextAndPreviousIcon imageSliderPrevious";
            H.style.top = (q.clientHeight - 35) / 2 + "px";
...
(function(d) {
    ...
    var e = (function() {
        ...
        var h = function(q, i) {
            ...
            var r = this;
            jm(this.container).bind("destroy", function(u) {
...
(function(e) {
    ...
    var f = ["01月", "02月", "03月", "04月", "05月", "06月", "07月", "08月", "09月", "10月", "11月", "12月"],
        b = ["日", "一", "二", "三", "四", "五", "六"],
        i = /^(\+|\-)?(\d+)(M|Y)$/i,
        d = function(k, l) {
    ...
    a.prototype = {
        ...
        _initDateInput: function(l, o) {
            ...
            e(k).on("click", function() {
                var r = m._options,
                    q = m._container || m.$el,
                    p = e.proxy(m._eventHandler, m);
                m.minDate(r.minDate).maxDate(r.maxDate).date(r.date || new Date()).refresh();
        ...
        _eventHandler: function(s) {
            var q = this._options,
                l = (this._container || this.$el).get(0),
                p, r, k, o, m;
            switch (s.type) {
                case "swipeLeft":
                case "swipeRight":
                    return this.switchMonthTo((s.type == "swipeRight" ? "-" : "+") + "1M");
                case "change":
                    m = e(".ui-calendar-header .ui-calendar-year, .ui-calendar-header .ui-calendar-month", this._container);
                    return this.switchMonthTo(h(m.eq(1)), h(m.eq(0)));
                default:
                    r = s.target;
                    if ((p = e(r).closest(".ui-calendar-calendar tbody a", l)) && p.length) {
                        s.preventDefault();
                        k = p.parent();
                        this._option("selectedDate", o = new Date(k.attr("data-year"), k.attr("data-month"), p.text()));
                        ...
                        if ((p = e(r).closest(".ui-calendar-prev, .ui-calendar-next", l)) && p.length) {
                            s.preventDefault();
                            this.switchMonthTo((p.is(".ui-calendar-prev") ? "-" : "+") + "1M")
                        } else {
                            if (((p = e(r).closest(".ui-calendar-return", l)) || (p = e(r).closest(".ui-calendar-cancel", l))) && p.length) {
                                s.preventDefault();
                                this._hideDate(this._container)
                            } else {
                                if ((p = e(r).closest(".ui-calendar-return", l)) && p.length) {
                                    s.preventDefault();
                                    this._hideDate(this._container)
                                } else {
                                    if ((p = e(r).closest(".ui-calendar-day", l)) && p.length) {}
                                }
                            }
                        }
                    }
        ...
        refresh: function() {
            var m = this._options,
                ...
                k = e.proxy(this._eventHandler, this);
            if (!m._invalid) {
                return
            }
        ...
        _generateHTML: function() {
            ...
            w += this._renderHead(k, r, v, t, o) + '<table  class="ui-calendar-calendar"><thead><tr>';
            for (u = 0; u < 7; u++) {
                A = (u + m) % 7;
        ...
        _renderHead: function(q, m, r, o, k) {
            ...
            if (q.yearChangeable) {
                ...
                for (p = Math.max(1930, m - 80), u = p + 100; p < u; p++) {
                    s += '<option value="' + p + '" ' + (p == m ? 'selected="selected"' : "") + ">" + p + "年</option>"
                }
                s += "</select>"
            } else {
            ...
            s += '</div><div class="ui-calendar-preandnext"><a class="ui-calendar-prev' + (o && o > l ? " ui-state-disable" : "") + '" href="#"></a><a class="ui-calendar-next' + (k && k < t ? " ui-state-disable" : "") + '" href="#"></a></div></div>';
            return s
        },
...
! function(a, c, b) {
    ...
    function i(f) {
        return k.call(g(f) ? f : function() {}, f, 1)
    }
        ...
        Animation: {
            ...
            _leaveTransforms: function(f) {
                var m = f.style[this._transitionPrefix + "Property"],
                    ...
                    p, g = b.coalesce(f.style.webkitTransform, f.style.MozTransform, f.style.transform),
                    i, o, k = a.parseInt(b.DOM.getStyle(f, "left"), 0),
    ...
    b.Carousel.CarouselClass = a({
        ...
        resetImagePosition: function(g) {
            ...
            var e = d.DOM.getAttribute(g, "src"),
                h, i, f, k, l, o = d.DOM.width(this.el),
